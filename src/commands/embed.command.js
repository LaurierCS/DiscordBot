...
async function execute(interaction) {
    ...
    if (isAdmin) {
        ...
        if (type == "rules") {
            ...
            const fields = [];
            for (const [emoji, roleId] of Object.entries(config.roleIds.reactionRoles.courses)) {
                const role = await server.roles.fetch(roleId);
                fields.push({ name: `${emoji}    ${role.name}`, value: "\u200B", inline: false });
            }

            const embed = coursesEmbed.embed;
            embed.fields = fields;
            ...
            await coursesChannel.send({ embed: embed });
            util.sendMessage("Sent the courses reaction role embed in its channel!", interaction);
